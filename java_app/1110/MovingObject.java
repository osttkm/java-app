...
abstract class MovingObject { // 抽象クラス
  ...
  MovingObject (int width, int height) {
    ...
    dx_Boss = (int) (Math.random()*10-5);    // -3から+3の範囲でランダム
    dy_Boss = (int) (Math.random()*10-5);    // -3から+3の範囲でランダム
  ...
  abstract void move(Graphics buf, int apWidth, int apHeight,int kilk_point);
